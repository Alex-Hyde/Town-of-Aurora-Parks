...
function Set-StorageBusProfile
{
    ...
    try
    {
        ...
        {
            New-ItemProperty -Path $clusbfltRegKeyPath -Name $systemFlagsKeyName -Value $systemFlags -Force | Out-Null
        }
        else
        {
...
function Get-StorageBusDisk
{
    ...
    switch ($psCmdlet.ParameterSetName)
    {
        ...
        "ByNumber"
        {
            $disks = $devices.Values | ? Number -match $Number
            break;
        }
        "ByPhysicalDisk"
        {
            ...
            try
            {
                ...
                if ($egPath.PathId -ne 0)
                {
                    ...
                    if ($mmPath.PathId -ne 0)
                    {
                        ...
                        $dm.SetDeviceAttributes($device.Guid, [Microsoft.SFS.StorageBusCache.ClusBflt.DiskAttribute]::Enabled, [Microsoft.SFS.StorageBusCache.ClusBflt.DiskAttribute]::All)
                    }
                    else
                    {
                        $errorObject = CreateErrorRecord -ErrorId "NotFound" `
